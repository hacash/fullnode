...
// root curr 
type RollerInsertRet = Ret<(Option<Arc<Chunk>>, Option<Arc<Chunk>>, leveldb::Writebatch)>;
...
pub fn insert_to_roller(roller: &mut Roller, parent: Arc<Chunk>, mut chunk: Chunk) -> RollerInsertRet {
    ...
    let mut tc_path = leveldb::Writebatch::new();
    if new_hei > curr_hei {
...
fn trace_upper_chunk(mut seek: Arc<Chunk>, upper_hei: u64, tc_path: &mut leveldb::Writebatch) -> Arc<Chunk> {
    let mut trc = |s: &Chunk| {
